  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto scroll-container p-4 grid gap-3 auto-rows-min">
        {campaigns.map((camp) => (
          <div key={camp.id} className="bg-bg-card border border-border rounded-xl p-4">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="font-semibold flex items-center gap-2">
                  🎯 {camp.name}
                  {camp.active === 0 && <span className="text-[10px] bg-accent-red/20 text-accent-red px-1.5 py-0.5 rounded">Pasif</span>}
                </h3>
                <div className="flex gap-2 mt-1">
                  <span className="text-xs bg-accent-purple/20 text-accent-purple px-2 py-0.5 rounded">{typeLabels[camp.type]}</span>
